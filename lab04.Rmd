...
# a grid of values for Na and Al at which to get the estimated class.
...
ggplot() +
  geom_raster(data = background_knn,
    mapping = aes(x = Na, y = Al, fill = est_party), alpha = 0.2) +
...
```{r}
#space to store cross-validation results 
...
for (i in seq_len(num_crossval_folds)){
  Cross_glass_train <- train_glass %>% slice(-crossval_val_fold_inds[[i]])
  Cross_glass_val <- train_glass %>% slice(crossval_val_fold_inds[[i]])
}
    
#train the model     
knn_fit_scaled_by_caret <- train(
  form = type ~ Al + Na, 
  data = Cross_glass_train,
  method = "knn",
  preProcess = "scale",
  trControl = trainControl(method = "none"),
  tuneGrid = data.frame(k=5))
    
#generate the prediction for the validation set data
y_hats <- predict(knn_fit_scaled_by_caret, newdata = Cross_glass_val, type = "raw")
head(y_hats)
#[1] WinF WinF WinF WinF WinF WinF
##Levels: Con Head Tabl Veh WinF WinNF

#the proportion of predicted parties are not equal to the observed party 
mean(y_hats != Cross_glass_val$type)
##[1] 0.4375
